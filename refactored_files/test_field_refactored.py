...
def test_write_to_runpath_produces_the_transformed_field_in_storage(snake_oil_field_example, storage):
    ...
    experiment_id = storage.create_experiment(parameters=ert.ensembleConfig().parameter_configuration)
    prior_ensemble = storage.create_ensemble(experiment_id, name='prior', ensemble_size=5)
    ...
    for real in [0, 3, 4]:
        permx_field.write_to_runpath(Path(f'export/with/path/{real}'), real, prior_ensemble)
        assert read_field(f'export/with/path/{real}/permx.grdecl', 'PERMX', permx_field.mask, Shape(permx_field.nx, permx_field.ny, permx_field.nz)).flatten().tolist() == pytest.approx(permx_field._transform_data(permx_field._fetch_from_ensemble(real, prior_ensemble)).flatten().tolist())
    for real in [1, 2]:
        with pytest.raises(KeyError, match=f"No dataset 'PERMX' in storage for realization {real}"):
            permx_field.write_to_runpath(Path(f'export/with/path/{real}'), real, prior_ensemble)
...
def egrid_file(tmp_path, grid_shape):
    grid = xtgeo.create_box_grid(dimension=(grid_shape.nx, grid_shape.ny, grid_shape.nz))
...
def test_field_dimensions_are_gotten_from_the_grid(parse_field_line, grid_shape):
    field = parse_field_line('FIELD PERMX PARAMETER permx.grdecl INIT_FILES:fields/perms%d.grdecl')
    assert (field.nx, field.ny, field.nz) == tuple(grid_shape)

@pytest.mark.parametrize('ext', ['roff_binary', 'roff_ascii', 'roff', 'grdecl', 'bgrdecl', 'ROFF_BINARY', 'ROFF_ASCII', 'ROFF', 'GRDECL', 'BGRDECL'])
...
@pytest.mark.parametrize('ext, expected', [('', "Line 3.*Missing extension for field output file 'param',.*'ROFF_BINARY', 'ROFF_ASCII', 'ROFF', 'GRDECL', 'BGRDECL'"), ('.wrong', "Line 3.*Unknown file format for output file: '.wrong'")])
def test_unknown_file_extensions_raises_config_validation_error(ext, expected, parse_field_line):
...
def test_unknown_transform_functions_raises_a_config_error(parse_field_line, transform):
    with pytest.raises(expected_exception=ConfigValidationError, match=f'Line 3.*FIELD {transform}:silly is an invalid function'):
        _ = parse_field_line(f'FIELD F PARAMETER f.grdecl INIT_FILES:f%d.grdecl {transform}:silly')
...
def test_invalid_forward_init_gives_a_user_error_message(parse_field_line, invalid):
    with pytest.raises(ConfigValidationError, match=f"Invalid boolean value '{invalid}'"):
        _ = parse_field_line(f'FIELD f PARAMETER f.roff INIT_FILES:f%d.grdecl FORWARD_INIT:{invalid}')

def test_missing_init_files_gives_a_user_error_message(parse_field_line):
    with pytest.raises(ConfigValidationError, match="Line 3.*Missing required INIT_FILES for field 'foo'"):
...
def test_the_user_gets_a_warning_about_input_transform_usage(parse_field_line):
    with pytest.warns(ConfigWarning, match='Got INPUT_TRANSFORM for FIELD: f, this has no effect and can be removed'):
        _ = parse_field_line('FIELD f parameter out.roff INPUT_TRANSFORM:log INIT_FILES:file.init')

@pytest.mark.parametrize('invalid_argument', ['doesnthavecolon', 'colonafter:', ':colonbefore'])
def test_invalid_argument_gives_a_user_error_message(parse_field_line, invalid_argument):
    with pytest.raises(ConfigValidationError, match=f"Line 3.*Invalid argument '{invalid_argument}'"):
        _ = parse_field_line(f'FIELD f parameter out.roff INIT_FILES:file.init {invalid_argument}')