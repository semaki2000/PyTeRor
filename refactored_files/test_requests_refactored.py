...
class TestRequests:
    ...
    @pytest.mark.parametrize(
        "parametrized_var_0, parametrized_var_1",
        [
            pytest.param("301", 301, id="test_http_301_changes_post_to_get"),
            pytest.param("302", 302, id="test_http_302_changes_post_to_get"),
            pytest.param("303", 303, id="test_http_303_changes_post_to_get"),
        ],
    )
    def test_http_301_changes_post_to_get_parametrized(
        self, httpbin, parametrized_var_0, parametrized_var_1
    ):
        r = requests.post(httpbin("status", parametrized_var_0))
        assert r.status_code == 200
        assert r.request.method == "GET"
        assert r.history[0].status_code == parametrized_var_1
        assert r.history[0].is_redirect
    ...
    @pytest.mark.parametrize(
        "parametrized_var_0, parametrized_var_1",
        [
            pytest.param("302", 302, id="test_http_302_doesnt_change_head_to_get"),
            pytest.param("303", 303, id="test_http_303_doesnt_change_head_to_get"),
        ],
    )
    def test_http_302_doesnt_change_head_to_get_parametrized(
        self, httpbin, parametrized_var_0, parametrized_var_1
    ):
        r = requests.head(httpbin("status", parametrized_var_0), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == "HEAD"
        assert r.history[0].status_code == parametrized_var_1
        assert r.history[0].is_redirect
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                ...
                id="test_should_strip_auth_host_change",
            ),
            pytest.param(
                "http://example.com:1234/foo",
                "https://example.com:4321/bar",
                id="test_should_strip_auth_port_change",
            ),
        ],
    )
    def test_should_strip_auth_host_change_parametrized(
        self, parametrized_var_0, parametrized_var_1
    ):
        s = requests.Session()
        assert s.should_strip_auth(parametrized_var_0, parametrized_var_1)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "http://user:pass@httpbin.org", "Basic dXNlcjpwYXNz", id="test_proxy_auth"
            ),
            pytest.param(
                "http://user:@httpbin.org", "Basic dXNlcjo=", id="test_proxy_auth_empty_pass"
            ),
        ],
    )
    def test_proxy_auth_parametrized(self, parametrized_var_0, parametrized_var_1):
        adapter = HTTPAdapter()
        headers = adapter.proxy_headers(parametrized_var_0)
        assert headers == {"Proxy-Authorization": parametrized_var_1}
